  ...
  <div class="body">
    ...
    <div id="simpleState" class="simple-state">
      <div class="panel">
        ...
        <button id="newRequestBtn" class="confirm-btn" style="display:none; width: 100%; margin-top: 16px;" onclick="iniciarNovoPedido()">Gerar novo pedido</button>
      </div>
    </div>
...
<script>
  ...
  function setSimpleState(state, titleText, detailText) {
    ...
    // Ocultar sempre a meta e os botões por padrão
    document.getElementById('meta').style.display = 'none';
    var retry = document.getElementById('retryBtn');
    var newReq = document.getElementById('newRequestBtn');
    retry.style.display = 'none';
    if (newReq) newReq.style.display = 'none';
    ...
    if (state === 'loading') { 
        ...
        icon.innerHTML = '✓'; 
        if (newReq) newReq.style.display = 'inline-block'; 
    }
    else if (state === 'err') { 
        icon.innerHTML = '!'; 
  ...
  function iniciarNovoPedido() {
    document.getElementById('avisarCliente').checked = true;
    carregarMensagens();
  }
